...
def sub_reconstructor(_, current_line, start_bytes):
...
def up_reconstructor(previous_line, current_line, _):
...
# 滤波的意义在于将数据平整，能压缩得更紧密
reconstruct_function = (empty, sub_reconstructor, up_reconstructor,
...
def un_filter_image_lines(image_lines, pixel_bytes):
    ...
    for _line in image_lines:
        ...
        reconstruct_function[_filter_mode](
...
def un_interlace(un_filter_images, width, height, pixel_bytes):
    # 下面方法，把每个图片的所有行当成一个一维数组来看待，也就有 7 个数组
    _reduced = tuple(itertools.chain(*_m) for _m in un_filter_images)

    for _h in range(height):
        _h_1 = (_h % 8) << 3  # 偏移量
        for _w in range(width):
            _image_order = adam7sub1[_h_1 + _w % 8]  # 当前图像的编号
...
def read_png(stream):
    ...
    while True:
        try:
            ...
            if _mask == HEADER:
                ...
                if _color_type == COLOR_TYPE_GRAY:
                    _extra_alpha = _chunk[1]  # 只取一半
                elif _color_type == COLOR_TYPE_RGB:  # 只取一半
                    _extra_alpha = _chunk[1], _chunk[3], _chunk[5]
                if _color_type == COLOR_TYPE_PALETTE:
                    # 这时候是一个 alpha table
                    _length = len(_chunk)
                ...
                if _color_type in (COLOR_TYPE_GRAY, COLOR_TYPE_GRAY_ALPHA):
                    _background = _chunk[1]
                elif _color_type in (COLOR_TYPE_RGB, COLOR_TYPE_RGB_ALPHA):
                    _background = _chunk[1], _chunk[3], _chunk[5]
                elif _color_type == COLOR_TYPE_PALETTE:
    ...
    # 按照交互标记确定是直线读取还是缩略图读取
    _adam = (adam7iter if _interlace else adam1iter)(_width, _height)
        ...
        try:
            ...
            if _line_bytes > 0:  # 大于 0 表示读取长度
                _reduced_images[_current_image].append(bytearray(
                    ...
                    read_iter(_unzip_data, _line_bytes * _pixel_bytes + 1))
                )
    ...
    _un_filter_images = (  # 把多个图片的打包在一起
        # 把一个图片的行打包在一起
        tuple(un_filter_image_lines(_image_lines, _pixel_bytes))
        for _image_lines in _reduced_images
    )

    # 数据回填
    if _interlace:
        _result = un_interlace(
    ...
    if _color_type == COLOR_TYPE_GRAY:
        if _extra_alpha:  # 如果有额外的 alpha 通道
            if _background:  # 把额外通道合并到灰度通道上，这比较简单
                ...
                _result = t_gray_to_rgb_alpha(_result, _extra_alpha)

        else:  # 不做任何处理
            _mode = 'gray'

    elif _color_type == COLOR_TYPE_GRAY_ALPHA:
        # 因为已经有了 alpha 通道，所以不关心 _extra_alpha
        _mode = 'full'
        _result = t_gray_alpha_to_rgb_alpha(_result, _background or 0)

    elif _color_type == COLOR_TYPE_PALETTE:
        if _extra_alpha:
            if _background:
                _mode = 'rgb'
                _result = t_palette_to_rgb(
                    _result, _palette, _extra_alpha, _background)
            else:
                _mode = 'full'
                _result = t_palette_to_rgb_alpha(
                    _result, _palette, _extra_alpha)

        else:
            _mode = 'rgb'
            _result = t_palette_to_rgb2(_result, _palette)

    elif _color_type == COLOR_TYPE_RGB:
        if _extra_alpha:
            if _background:
                _mode = 'rgb'
                _result = t_rgb_to_rgb(_result, _extra_alpha, _background)

            else:
                _mode = 'full'
                _result = t_rgb_to_rgb_alpha(_result, _extra_alpha)

        else:  # 不做任何处理
            _mode = 'rgb'

    elif _color_type == COLOR_TYPE_RGB_ALPHA:
        if _background:
            _mode = 'rgb'
            _result = t_rgb_alpha_to_rgb(_result, _background)
        else:
            _mode = 'full'
...
def write_png(width, height, mode, data, data_split=40503):
    if mode == 'gray':
        _color_type = COLOR_TYPE_GRAY
        _row_bytes = width
    elif mode == 'rgb':
        _color_type = COLOR_TYPE_RGB
        _row_bytes = width * 3
    elif mode == 'full':
        _color_type = COLOR_TYPE_RGB_ALPHA
        _row_bytes = width * 4
    else:
        raise

    _head_data = struct.pack('!2I5B', width, height, 8, _color_type, 0, 0, 0)
    _head_crc32 = struct.pack('!I', zlib.crc32(HEADER + _head_data))
    ...
    for _s_data in _split_data:
        ...
        _s_crc32 = struct.pack('!I', zlib.crc32(DATA + _s_data))
